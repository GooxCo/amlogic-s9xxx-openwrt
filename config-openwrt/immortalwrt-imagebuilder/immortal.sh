...
imagebuilder_path="${make_path}/${immortalwrt_dir}"
custom_files_path="${make_path}/config-immortalwrt/immortalwrt-imagebuilder/files"
custom_config_file="${make_path}/config-immortalwrt/immortalwrt-imagebuilder/config"
...
download_imagebuilder() {
    cd ${make_path}
    echo -e "${STEPS} Start downloading immortalwrt files..."
    ...
    mv -f immortalwrt-imagebuilder-* ${immortalwrt_dir}
...
rebuild_firmware() {
    cd ${imagebuilder_path}
    echo -e "${STEPS} Start building immortalwrt with Image Builder..."
    ...
    my_packages="\
        ...
        f2fs-tools f2fsck fdisk gawk getopt git-http gzip hostapd-common iconv iw iwinfo jq jshn \
        ...
        uuidgen wget-ssl whereis which wireless-tools wpa-supplicant hostapd px5g-wolfssl wwan xfs-fsck xfs-mkfs xz \
...
echo -e "${STEPS} Welcome to Rebuild immortalwrt Using the Image Builder."
[[ -x "${0}" ]] || error_msg "Please give the script permission to run: [ chmod +x ${0} ]"
[[ -z "${1}" ]] && error_msg "Please specify the immortalwrt Branch, such as [ ${0} 21.02.3 ]"
rebuild_branch="${1}"
